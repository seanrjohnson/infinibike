...
`WorldScene` manages the camera, atmosphere, ride updates, and chunk lifecycle. `ChunkBuilder` composes road, city, and countryside detail through `RenderContext`. `TerrainStream` separately owns spatial terrain, water, decorative road continuations, and distant scenery. The scenery planner receives the generator and surface, never graphics quality, asset loading state, or the camera.
...
`TerrainSurface` retains the 5 m placement heightfield and triangle interpolation used by roads, buildings, actors, and shorelines. `TerrainStream` selects absolute 50 m tiles in every direction around the rider, independent of route-chunk lifetime. Tiles are batched into 800 m pages to bound draw calls and geometries. Interior spacing is 5/10/25 m at initial 250/600 m distance bands, with 50 m hysteresis. Tile boundaries always retain the same 5 m vertices; triangle fans stitch coarse interiors to those edges. Replacement interior heights morph from the previous triangulation over 350 ms, while boundary heights stay fixed. Detailed corridors, accepted scenery footprints, and decorative streets force fine tiles. Water moves with spatial terrain and clips to the same fine grid. Legacy chunk mesh helpers remain for surface contract tests, but runtime road chunks no longer own ground or water.

Beyond the existing placement corridor, terrain blends over 100 m into seeded world-coordinate hills. The procedural sky remains camera-centered; hills, woodland clusters, settlements, and district-colored city silhouettes stay in absolute world coordinates and therefore exhibit geometric parallax. The existing 1,800 m far plane and weather visibility distances remain unchanged. Repeated trees and buildings share primitive geometry/materials, with separately disposed instance buffers and no distant shadows. Detailed terrain still receives shadows.

Decorative city streets are rays from the existing intersection arms. Countryside continuations retain the curved fork entrance and extend its straight arm as a ray. Rays are clipped into the active spatial tiles, so there is no visible endpoint tied to the source chunk. Their footprints reserve distant scenery clearance. They have no effect on ridden routes, grade, trainer behavior, or discoveries.

Spatial generation prepares a consistent 6 km route look-ahead before sampling; this does not change the route generator. Terrain and scenery GPU resources retire with pages. Existing route-generation history remains CPU state, as before. This look-ahead is a practical finite neighborhood, not a guarantee against an arbitrarily distant future route looping back into previously visited terrain.
...
Paused scenes, menus, and frozen QA scenes render on demand. Asset readiness, camera/quality changes, resize events, and pending terrain work invalidate frames. Active rides retain the animation loop. Terrain construction yields between tiles with a nominal 2 ms CPU allowance per frame; a single tile or final page upload can exceed that soft allowance. New coverage has priority over refinements, and existing pages remain until their replacement is complete. A lowered coarse safety mesh covers newly requested space while pages build and is disposed when the queue settles. Initial configuration completes terrain before showing scenery, avoiding floating objects over the temporary cover. QA `freeze()`, `setDistance()`, and `settleExpansion()` also drain terrain work for deterministic captures; subsequent active riding stays incremental. Once work and morphs settle, idle scenes stop submitting frames.
...
## Expansive-world review tools

- `node tools/expansion-captures.mjs before http://127.0.0.1:4173 --full` records 162 matched views: countryside forks, bends, lakeshores, highland terrain, city turns, and city hills; three approach phases, three camera modes, and three angles. Run the same command with `after` against the changed renderer. The fixture manifest records exact distances and renderer diagnostics. The fixture seed is `windows-visual-qa`; medium quality and the default clear/golden lighting are identical in both runs.
- `node tools/expansion-review.mjs before after` produces `test-results/expansion-review.html`, a filterable side-by-side gallery linking full-resolution images. All generated captures remain untracked.
- `tests/e2e/world-expansion.spec.ts` checks daytime turn approaches, spatial retirement, repeated journeys, and clear/cloudy/rainy night coverage on desktop, phone, and landscape tablet. Existing graphics tests retain raw-canvas color checks and failed-asset coverage.
- Expansion diagnostics include tile counts by LOD, page count, terrain triangles, distant instance count, pending page builds, last construction duration, continuation/page intersections, and coverage radius. Coverage includes a 200 m margin for the 100 m guard, camera offset, and movement between tile selection updates. Quality changes reduce distant instances while retaining coverage.

The sky keeps its procedural sun, stars, and gradient. Its horizon uses the current regional fog color without tone mapping, matching the renderer's fog output; the upper gradient and celestial glow fade into that color at grazing angles. This avoids a contrasting band exposing the far ground boundary.

The existing graphics probe uses explicit QA distance changes. Those calls now settle terrain for deterministic captures, so its CPU submission measurements include draining queued terrain work; normal riding instead advances construction incrementally. Treat those measurements as capture costs, not a physical-device frame-rate measurement.

### Expansion validation, September 21, 2026

`npm run check` passes with 240 unit tests, ESLint, Prettier, TypeScript, and the production build. The existing Vite bundle-size advisory remains. The full browser run initially recorded 140 passes, 10 skips, and 10 failures. Three failures exceeded the geometry ceiling by 1–3 geometries; merging ground, water, and decorative roads into one page buffer resolved them. Removing redraws for unfinished, invisible tile work resolved the summary/settings timing failures. The focused rerun passed 14 of 16 cases, including every affected geometry and summary/settings case on desktop, phone, and tablet.

The remaining two failures are phone discovery fixtures in `discoveries.spec.ts` (journal encounter polling and rain/reduced-motion spectral-deer visibility). Both reproduce at the same assertions in an isolated copy with the original chunk-owned terrain and no expansion renderer. Their landscape-tablet counterparts pass. These are pre-existing phone-view fixture failures; the full suite is therefore not reported as wholly green. Existing unrelated discovery/UI work was retained.

The final graphics/expansion/opt-in run passed 28 of 29 cases; its only failure was an older wildlife fixture looking for a legacy dinosaur in version-two worlds. The fixture now checks current countryside wildlife (cow). After the final sky and startup corrections, all 11 affected desktop/phone/tablet visual cases pass, including medium/high 1440p, wildlife, and clear/cloudy/rain night views. A concurrent check attempt hit four unit-test timeouts and one native WebGL context loss; sequential reruns passed without relaxing assertions.

The matched clear/golden matrix contains 162 before and 162 after images in `test-results/expansion-before` and `test-results/expansion-after`, with the filterable comparison at `test-results/expansion-review.html`. Maximum after-capture diagnostics were 3,525,760 triangles, 1,240 draw calls, 662 geometries, 3 textures, and 36 spatial pages; all captures had an empty construction queue. Baseline maxima were 2,828,398 triangles, 1,219 calls, 613 geometries, and 3 textures. Representative fork, lakeshore, elevated, and city-turn views were inspected: continuous surroundings replace exposed corridor edges and decorative streets extend into the background. Complete human review of the gallery and physical-device motion remains necessary for final immersion acceptance.

Sequential same-host graphics probes are saved under `test-results/performance-expansive-before` and `test-results/performance-expansive-after`, including continuous travel videos and measurement JSON. The 240?270 m medium-quality capture sequence measured:

| Scene       | Median capture interval, before / after | P95 interval, before / after | Median CPU submission, before / after | Maximum CPU submission, before / after |
| ----------- | --------------------------------------- | ---------------------------- | ------------------------------------- | -------------------------------------- |
| City        | 507.7 / 540.8 ms                        | 540.4 / 571.0 ms             | 3.9 / 4.1 ms                          | 222.4 / 934.3 ms                       |
| Countryside | 410.5 / 424.3 ms                        | 423.3 / 753.2 ms             | 2.0 / 2.4 ms                          | 84.4 / 526.6 ms                        |

After-probe city resources were 2,324,397 triangles, 1,177 calls, and 624 geometries; countryside used 2,586,196 triangles, 502 calls, and 218 geometries. Both used three textures and had zero context losses. The larger submission spikes include the new synchronous QA queue drain, so these results do not establish normal-frame streaming cost or the 30 FPS hardware target. Initial configuration also completes coverage synchronously; improving its latency remains a performance opportunity.

Physical Galaxy Tab A9+ frame rate and physical trainer behavior remain **untested**. No road-generation, ride-physics, Bluetooth-write, storage-schema, or deployment changes are part of this expansion.
...
Chunk detail now selects the scenery representation: the current chunk and two ahead retain authored detail; further chunks use simplified silhouettes without small props or shadows. Outer rows remain simplified even when their owning chunk is near. Detailed roadside triangulation stays fixed; spatial outer terrain now uses the LOD policy described above. Lightweight forward road coverage reaches the weather fog distance. Detail transitions build a replacement before retiring the previous resources.
...
## Landmark surroundings and neighborhood character

Accepted monuments now open deterministic approach views. Surroundings include biome-specific furniture, framing trees, broken columns, short stone walks and terraces, and longer walks terminating in small open pavilions. Complete walks are omitted if any member fails clearance or support; foundation relief is capped at 0.65 m and adjacent support changes at 0.4 m. Low crossing gardens stay beyond the span footprint. The island-abbey boat landing has a canopy and explicit piling footprints. All additions use shared primitives, chunk-owned materials, ordinary distance simplification, and existing disposal. No extra actors, lights, or rideable roads are introduced.

Arcaded City uses four neighborhood identities over 750 m sections with a 100 m parcel-stable transition. Each shares palette, arcade bays, crown style, height character, and street setback. Frontage parcels retain the shuffled archetype deck; courtyard and skyline parcels select forms appropriate to their role. Individual parcel dimensions remain seeded and all final placements pass normal road, intersection, overlap, and terrain checks. The new neighborhood metadata is limited to version-two procedural scenery. New platform validation remains tablet-only.

All seven city biomes now use this neighborhood system: four identities per biome (28 total), with role-specific forms in the five ordinary districts and Brutalist Gardens. Corner-role parcels accent their height and bay rhythm; courtyards and skylines keep each district's purpose recognizable. Neighborhood changes reuse existing architecture parts and materials, without additional actors, lights, geometry types, or resource ownership. Full parcel dimensions include neighborhood depth changes before support/overlap checks; street setbacks pass through the existing road and intersection exclusions. Ordinary asset-library buildings keep their existing appearance and placement. The 3 km identity cycle does not repeat individual building signatures.

## Biome life and ground details

`scenic-detail.ts` selects occasional themed prop replacements using a separate seeded hash, with no extra draws from road or ordinary scenery randomness. `scenic-detail-renderer.ts` builds shared-primitive walls, gates, irrigation channels, column fragments, reeds, planted steps, and footbridges, plus five animated encounter types. The planner reserves complete geometry/movement envelopes and rejects roads, water, overlaps, and excessive foundation relief. Ground footings reach the lowest sampled terrain; moving terrestrial actors sample ground beneath their local position.

Encounters stream with scenery chunks. They are extracted before static batching (including the city's second batch pass), and nearest complete groups share a quality-dependent budget of 6/12/18 actors, within 350 m. Existing actors retain their separate budgets. Reduced motion holds a deterministic pose. Distant chunks omit prop details. Geometry uses the world primitive pool, materials use the existing chunk cache, and retirement follows normal chunk disposal. No new point lights, external assets, settings, or history migrations are introduced. Version 1 replay excludes these details; version 2 retains its selected biome mix with the updated scenery recipes.

Channels and deer drinking basins are contained decorative water surfaces, independent of lake generation. Footbridges cross these small channels only. No added paths are rideable.

## Restrained events and encounter capture

`scenic-detail.ts` supplies a separate hashed 1.5 km opportunity stream for three rare events. They share `LifeGroup` extraction, cached primitives/materials, actor caps, terrain sampling and chunk disposal. Their parcels take precedence over ordinary vegetation, reserve a 7 x 5 x 5 m envelope, and allow 1.8 m ground relief. The `WorldScene` activation cache tracks a 60-second lifetime independently of rebuilt render groups and retires records beyond 1.5 km. Reduced motion uses a fixed representative pose; pausing does not advance time. No point lights or transparent particle systems are added.

`discovery-catalog.ts` composes monumental families from the existing exported decks and adds event, life and landscape families. Once per second after a rendered riding frame, `WorldScene` checks accepted nearby sites and visible actors against the camera frame and a sampled terrain sightline. This deliberately avoids an expensive full-scene mesh raycast and does not promise perfect foreground-object occlusion. The application captures a subject-framed, downsampled JPEG for each new family, capped at three captures per second, then stores progress through the validated, bounded domain journal. The render loop does not read or write storage directly. The journal stores seed/settings for existing route replay; no route/terrain changes or resistance-control code is involved.
