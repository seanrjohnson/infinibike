...
for (const mobile of [false, true]) {
  test(
    "shows Dreamwood monuments throughout a long ride " +
    ...
    async ({ page }, testInfo) => {
      ...
      for (const form of MONUMENT_FORMS) {
        ...
        for (const quality of ["low", "high", "medium"] as const) {
          ...
          if (form === "floating-monastery" || form === "whale-conservatory") {
            const motion = await page.evaluate(() => {
              const qa = window.__INFINIBIKE_VISUAL_QA__!;
              const before = qa
                .monumentFrames()
                .filter((frame) => frame.kind === "drift" && frame.active);
              qa.advanceActors(3);
              return {
                before,
                after: qa
            ...
            expect(motion.before.length).toBeLessThanOrEqual(
              quality === "low" ? 2 : quality === "medium" ? 4 : 6,
            );
            expect(motion.after.map((frame) => frame.position)).not.toEqual(
              motion.before.map((frame) => frame.position),
            );
          }
