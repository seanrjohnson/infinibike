...
for (const mobile of [false, true]) {
  test(
    "shows wildlife observatory variants throughout a long ride " +
    ...
    async ({ page }, testInfo) => {
        ...
        for (let index = 0; index < 120; index++) {
          results.push(
            ...qa
              .scenery(index)
              .filter(
                (item) => item.architecture?.form === "wildlife-observatory",
              ),
          );
          if (index % 4 === 0)
      ...
      await page.screenshot({
        path: testInfo.outputPath("observatory-passing.png"),
      });
